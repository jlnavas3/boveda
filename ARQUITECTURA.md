...
- **Abecedario Lateral (Fast-Scroller Niagara):** `mostrarIndiceAlfabetico`, `indiceEfectoOla`, `indiceAmplitudOlaDp` (109 dp), `indiceRadioOlaDp` (169 dp), `indiceEscalaLetras` (1.5x), `indiceMostrarCirculo`, `indiceTamanoCirculoDp` (50 dp), `indiceOffsetCirculoDp` (136 dp), `indiceHaptica`, `indiceAnchoTactilDp` (45 dp), `indiceTonoLetras` (80%), `indiceIncluirEnie` (true), `indiceResaltarEntradas` (true), `indiceResaltarSoloPrimera` (true).
...
- `IndiceAlfabetico`: Fast-scroller táctil con curvatura fluida estilo Niagara Launcher:
  ...
  - **Captura Gestual de Área Amplia y Resaltado en Tiempo Real Personalizable:** El contenedor `Box` define un ancho `anchoZonaTactilDp` (45 dp por defecto, configurable de 26 a 90 dp) y consume el evento inicial (`down.consume()`). Emite la letra activa mediante `alCambiarLetraActiva(letra)` durante el arrastre, permitiendo el resaltado en tiempo real de la primera entrada (modo ancla) o de todas las entradas coincidentes, con opción de desactivado completo.
  ...
  - **Burbuja Flotante de Cresta Configurable:** Diámetro personalizable (`indiceTamanoCirculoDp`, 50 a 110 dp, 50 dp por defecto) sin bordes, fondo con degradado ámbar, elevación de 14 dp, escala proporcional de tipografía y proyección hacia el centro de la pantalla (`indiceOffsetCirculoDp`, 136 dp por defecto) completamente desacoplada del ancho de arrastre táctil.
...
- **`PantallaAjustesIndice`**: Pantalla modular e independiente que aloja toda la personalización visual y táctil del abecedario (amplitud, radio, escala, offset de burbuja, zona táctil, tono de brillo, interruptores de ola, círculo, vibración háptica e inclusión de Ñ), tarjetas de muestra interactivas y la barra lateral táctil real anclada al extremo derecho para comprobación a pantalla completa.
